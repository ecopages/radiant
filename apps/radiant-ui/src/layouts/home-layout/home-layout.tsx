import { eco } from '@ecopages/core';
import type { JsxRenderable } from '@ecopages/jsx';
import radiantUiJson from '../../../../../packages/radiant-ui/package.json';

export type HomeLayoutProps = { children: JsxRenderable };

const GitHubIcon = () => (
	<svg width="24" height="24" viewBox="0 0 98 96" xmlns="http://www.w3.org/2000/svg" fill="currentColor" aria-hidden="true">
		<path d="M48.854 0C21.839 0 0 22 0 49.217c0 21.756 13.993 40.172 33.405 46.69 2.427.49 3.316-1.059 3.316-2.362 0-1.141-.08-5.052-.08-9.127-13.59 2.934-16.42-5.867-16.42-5.867-2.184-5.704-5.42-7.17-5.42-7.17-4.448-3.015.324-3.015.324-3.015 4.934.326 7.523 5.052 7.523 5.052 4.367 7.496 11.404 5.378 14.235 4.074.404-3.178 1.699-5.378 3.074-6.6-10.839-1.141-22.243-5.378-22.243-24.283 0-5.378 1.94-9.778 5.014-13.2-.485-1.222-2.184-6.275.486-13.038 0 0 4.125-1.304 13.426 5.052a46.97 46.97 0 0 1 12.214-1.63c4.125 0 8.33.571 12.213 1.63 9.302-6.356 13.427-5.052 13.427-5.052 2.67 6.763.97 11.816.485 13.038 3.155 3.422 5.015 7.822 5.015 13.2 0 18.905-11.404 23.06-22.324 24.283 1.78 1.548 3.316 4.481 3.316 9.126 0 6.6-.08 11.897-.08 13.526 0 1.304.89 2.853 3.316 2.364 19.412-6.52 33.405-24.935 33.405-46.691C97.707 22 75.788 0 48.854 0z" />
	</svg>
);

const RadiantLogo = () => (
	<a href="/" class="ui-logo" title="Radiant UI" aria-label="Radiant UI home">
		<svg height="32" viewBox="0 0 35 42" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
			<path
				d="M8.95038 19.0355C8.73635 19.7972 10.4934 24.8245 18.2298 23.3884C25.3299 22.0704 24.7298 10.8884 18.2298 13.8884C14.2596 15.7208 12.2883 33.409 22.2298 31.8884C29.9216 30.7119 28.75 6.88572 31.434 1.38842C28.4393 4.49823 5.89504 5.06009 2.22981 17.8884C1.27183 21.2414 3.22979 26.8884 8.95036 28.3884L2.22981 38.3884"
				stroke="currentColor"
				stroke-width="4"
				stroke-linecap="round"
			/>
		</svg>
		Radiant UI
	</a>
);

export const HomeLayout = eco.component<HomeLayoutProps, JsxRenderable>({
	dependencies: {
		stylesheets: ['./home-layout.css', '../../components/logo/logo.css', '../base-layout/base-layout.css'],
		scripts: ['../base-layout/base-layout.script.ts', '../../components/theme-toggle/theme-toggle.script.ts'],
	},
	render: ({ children }) => (
		<body>
			<header class="site-header">
				<div class="site-header__inner">
					<div class="site-header__start">
						<RadiantLogo />
						<p class="site-header__version">v {radiantUiJson.version}</p>
					</div>
					<nav class="site-header__nav" aria-label="Site">
						<ul>
							<li>
								<a href="/docs/introduction">Docs</a>
							</li>
							<li>
								<a href="https://github.com/ecopages/radiant" aria-label="GitHub repository">
									<GitHubIcon />
								</a>
							</li>
							<li>
								<theme-toggle id="toggle-dark-mode">
									<span class="sr-only">Theme</span>
								</theme-toggle>
							</li>
						</ul>
					</nav>
				</div>
			</header>
			<main class="ui-home-layout">
				<div class="ui-home-layout__inner">{children}</div>
			</main>
		</body>
	),
});
