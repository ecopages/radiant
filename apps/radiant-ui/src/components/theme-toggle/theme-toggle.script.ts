...
/**
 * @remarks
 * Persists preference in `localStorage`, mirrors `prefers-color-scheme` when unset,
 * and broadcasts `eco:theme-change` so a freshly mounted toggle stays in sync.
 */
...
export class ThemeToggle extends RuiSwitchElement {
	...
	onThemeChange(event: CustomEvent<ThemeChangeDetail>) {
		...
		if (this.checked !== isDark) this.applyTheme(isDark);
	}
	...
	private handleThemeChange() {
		...
		window.dispatchEvent(new CustomEvent<ThemeChangeDetail>(THEME_CHANGE_EVENT, { detail: { theme, isDark } }));
	}
...
export type ThemeToggleProps = RuiSwitchProps & { id?: string };
