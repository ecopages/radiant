...
The default composition accepts `startLabel` and `endLabel` view props for the two segment editors. Use those props when the default structure is sufficient; set `aria-label` directly on `RuiDateRangePickerStartInput` and `RuiDateRangePickerEndInput` when composing the children yourself.

Typing one side does not commit the range. The host `value` updates when both start and end are valid ISO dates, or when a previously committed range is cleared.
...
<rui-date-range-picker value="2026-08-01/2026-08-14">
  <div class="rui-date-range-picker__group">
    <rui-date-input data-range-start aria-label="Start date"></rui-date-input>
    <rui-date-input data-range-end aria-label="End date"></rui-date-input>
...
`RuiDateRangePicker` is a custom element (`<rui-date-range-picker>`) that coordinates its light-DOM inputs, toggle button, and range calendar popover. It is not form-associated; `start-name` / `end-name` land on the nested `rui-date-input` hosts. `RuiField` `name` is the store key for the combined `start/end` value.
...
| `start-name` | `string` | `''` | Form `name` on the nested start `rui-date-input`. |
| `end-name` | `string` | `''` | Form `name` on the nested end `rui-date-input`. |
...
| --- | --- | --- | --- |
| `[data-range-start]` | yes | `name`, `locale`, `min`, `max`, `disabled`, `read-only`; `value` only for a complete range | nested `rui-date-input` |
| `[data-range-end]` | yes | same as start | nested `rui-date-input` |
...
| --- | --- | --- |
| `rui-change` | `{ value, start, end }` | Fired when a valid range is committed, or when a committed range is cleared. |
...
| `.rui-date-range-picker__input` | Host for nested `rui-date-input`. |
...
- Give both segment groups distinct accessible names (`aria-label` or `startLabel` / `endLabel` on the default composition).
- Segments use spinbutton semantics with numeric keypads on touch devices.
