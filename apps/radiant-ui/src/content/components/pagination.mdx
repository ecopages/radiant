...
| `.rui-pagination--compact` | Force previous / page position / next chrome (also used below `40rem`). |
| `.rui-pagination__status` | Wrapper for the compact page-position label. |
| `.rui-pagination__status-label` | Muted compact label (`Page n of m`); hidden when numbered pages are shown. |
