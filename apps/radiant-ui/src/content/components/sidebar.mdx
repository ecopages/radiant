...
Each trigger also reflects its controlled sidebar's state, mobile mode, and collapse mode on the trigger host. Placement styles read those local attributes and the `placement` attribute, including when providers are nested. Before a trigger attaches to its sidebar (the server-rendered paint), placement styles fall back to the provider's primary sidebar.
...
The host writes `data-sidebar-state`, `data-sidebar-mobile`, and `data-sidebar-collapsible` on `<rui-sidebar-trigger>`. `data-sidebar-mobile` and `data-sidebar-collapsible` appear once the trigger attaches to its sidebar.
