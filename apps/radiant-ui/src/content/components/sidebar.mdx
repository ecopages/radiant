...
Each trigger also reflects its controlled sidebar's state, mobile mode, and collapse mode on the trigger host. Placement styles use those local attributes, including when providers are nested.
...
| `placement` | `''` · `header` · `inset` | `''` | Trigger placement. |
| `variant` | `ghost` · … | `ghost` | Button tone (reuses `RuiButton` variants). |
| `size` | `sm` · `md` · `lg` | `md` | Button size. |

The host writes `data-sidebar-placement`, `data-sidebar-state`, `data-sidebar-mobile`, and `data-sidebar-collapsible` on `<rui-sidebar-trigger>`.
