...
Each trigger also reflects its controlled sidebar's state, mobile mode, and collapse mode on the trigger host. Placement and glyph styles read only the trigger host and its `placement` attribute, so an attached trigger follows the sidebar's `mobileBreakpoint` rather than the viewport width, including when providers are nested.
