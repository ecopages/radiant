...
`<rui-alert>` coordinates any light-DOM tree that matches its query contract. The `Rui*` helpers stamp these targets; they are not required.

```tsx
import '@ecopages/radiant-ui/alert';

<rui-alert dismissible close-label="Dismiss">
  <div role="alert" class="rui-alert rui-alert--warning rui-alert--inline">
    Your session will expire in 5 minutes.
    <button type="button" data-alert-close class="rui-alert__close" aria-label="Dismiss"></button>
  </div>
</rui-alert>
```

BEM classes are presentation-only. The host only queries `[data-alert-close]`; authors own the `role="alert"` surface.
...
| --- | --- | --- | --- |
| `[data-alert-close]` | no | — | presence, `aria-label` |

The host does not query alert content. Authors own the `role="alert"` surface (or use `RuiAlert`). Nested hosts: none.
...
| Method | Description |
| --- | --- |
| `dismiss()` | Emit `rui-close`, then remove the host from the DOM. |

### View helpers

| Component | Target stamped | Notes |
| --- | --- | --- |
| `RuiAlert` | `<rui-alert>` + inner `role="alert"` surface | Appends `[data-alert-close]` when `dismissible`. |
| `RuiAlertIcon` | — | Presentation only (`inline` layout). |
| `RuiAlertTitle` | — | Presentation only (`banner` layout). |
| `RuiAlertDescription` | — | Presentation only (`banner` layout). |
