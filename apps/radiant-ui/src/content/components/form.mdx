...
- Form-associated hosts (`name` on the host): `rui-date-input`, `rui-number-field`, `rui-slider`, `rui-knob`. Empty date values are omitted. A range slider also submits `{name}-max`. Number-field submits the raw number, not the formatted display
...
Custom hosts: `registerFieldControl` for the store. `FormAssociation` plus `static get formAssociated()` to list on a native form. Import `FormAssociation` from `@ecopages/radiant-ui/form`. Named form-associated hosts reset and disable with the native `<form>` like `<input>`.
