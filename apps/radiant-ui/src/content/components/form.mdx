...
<p>Every `RuiField` child should be a Radiant control (`RuiInput`, `RuiSelect`, `RuiDateInput`, …). The form store discovers values through the Field control protocol; unmarked native inputs are not registered.</p>
...
Custom hosts: call `registerFieldControl(tag, { read, write, submission })` before connecting a field. `submission` is `native` by default (name the native ARIA target), `host` for a form-associated host, or `none` for a store-only control. Set `data-rui-aria-target` on the host when the native input is inside it. Registration does not make a host form-associated: the host must set `formAssociated`, call `attachInternals()`, and update `setFormValue()`; `FormAssociation` from `@ecopages/radiant-ui/form` wraps the latter two steps. The catalog hosts support native submission, fieldset disability, and reset. Browser validity and state restoration require separate host behavior; `RuiForm` rules validate through its store.

## Default values

<p>Pass `defaultValues` to pre-populate edit forms. Fields read their initial state from the form store. Multi-value controls (`RuiSelect`, `RuiCombobox`, `RuiListbox`, `RuiCheckboxGroup`, `RuiTagGroup`) store `string[]` — seed them with an array (`{ language: ['ts'] }`). A CSV string still writes because the host parses it; after that the stored value is an array. `RuiSlider` stores `number[]`.</p>
