...
import { meta as CalendarMeta, Default, Multiple, Range } from '@/content/stories/calendar';
...
`<rui-calendar>` is a **Derived Tree** host: it `render()`s the month grid from `value`, `min`, `max`, and `selection-mode`. You do not stamp `data-calendar-*` targets yourself. Parent hosts (`rui-date-field`, `rui-date-range-picker`) embed `<rui-calendar>` and query `[data-calendar-day]` inside it when opening a popup.

## Selection modes

<p>`single` selects one date. `multiple` accepts comma-separated values. `range` expects `start/end` in `value`.</p>

<Canvas of={Multiple} meta={CalendarMeta} />
<Canvas of={Range} meta={CalendarMeta} />
...
| --- | --- | --- |
| `[data-calendar-prev-month]` / `[data-calendar-next-month]` | `disabled` | paging controls |
| `[data-calendar-grid]` | `role="grid"` | month grid |
| `[data-calendar-day]` | `data-iso`, `tabIndex`, `aria-selected`, `disabled` | day buttons |
| `[data-calendar-today]` | `disabled` | jump to today |

Parent hosts query `[data-calendar-day][data-iso="…"]` for popup focus.
