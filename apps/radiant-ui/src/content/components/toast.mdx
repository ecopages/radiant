...
`<rui-toaster>` and `<rui-toast>` are **Derived Tree** hosts: the toaster `render()`s `.rui-toaster` and `rui-toast` children; each toast `render()`s `[data-toast-close]` and `[data-toast-action]`. Use the imperative `toast()` API — do not author these targets by hand.

The toaster queries `.rui-toaster` (stack list class) and `rui-toast` for layout math.
