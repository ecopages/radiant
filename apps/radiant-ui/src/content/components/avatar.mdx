...
Avatar chrome maps to **semantic surface roles**, never Tailwind palette steps. Override `--rui-avatar-*` on `.rui-avatar`. Size classes set `--rui-avatar-size`.

| Part | Default | Override |
| --- | --- | --- |
| Size | `--space-10` (`md`); `sm` `--space-8`; `lg` `--space-12` | `--rui-avatar-size` |
| Fill / radius | `--surface-container`, `--radius-pill` | `--rui-avatar-surface`, `--rui-avatar-radius` |
| Fallback text | `on-surface` | theme |

```css
.rui-avatar {
  --rui-avatar-size: 2.25rem;
}
```
