...
<rui-number-field value={3} min-value={0} max-value={10}>
...
BEM classes are presentation-only. The host owns formatted input `value` and spinbutton `aria-*`.
...
| --- | --- | --- | --- |
| `[data-number-field-input]` | yes | `id`, `role`, `inputmode`, `data-disabled`, `data-readonly`, `aria-valuemin`, `aria-valuemax`, `aria-valuenow`, formatted `value` | text input; stamp `data-rui-control` for field wiring |
| `[data-number-field-value]` | no | `value`, `name`, `disabled` | hidden form input |
| `[data-number-field-action="decrement"]` | no | `disabled`, `aria-label` | decrement button |
| `[data-number-field-action="increment"]` | no | `disabled`, `aria-label` | increment button |
...
| --- | --- | --- |
| `RuiNumberField` | default group when `children` omitted | Includes input, hidden value, and steppers. |
...
| `RuiNumberFieldSteppers` | stepper column wrapper | Presentation only. |
