...
`<rui-listbox>` is a **view-owned shell**: `RuiListboxOption` children stay in parent JSX. The custom element coordinates `aria-selected`, roving tabindex, and `rui-change`. When `embedded` is set, the parent (select or combobox) owns selection and keyboard; the listbox only paints options.
...
| `RuiListboxOptionIndicator` | Decorative selected-state indicator; children replace the default check SVG. |
