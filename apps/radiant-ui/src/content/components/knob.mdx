...
import { meta as KnobMeta, Default } from '@/content/stories/knob';
...
Fractional `step` values are rounded to the step's decimal places so committed values stay clean (`0.3` instead of `0.30000000000000004`). Set `precision` to cap fraction digits, or pass `parseValue` for custom rounding.
...
`parseValue` is a function prop (`(value: number) => number`), not an attribute. Pass it to `RuiKnob`.
