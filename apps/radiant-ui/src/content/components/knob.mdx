...
<rui-knob value={50} min={0} max={100} label="Gain">
  <div class="rui-knob" data-ref="root">
    ...
    <button type="button" class="rui-knob__control" data-ref="control" data-knob-control role="slider">
      ...
      <span class="rui-knob__value" data-ref="centerValue" aria-hidden="true"></span>
    </button>
...
BEM classes are presentation-only. The host owns control `aria-*`, ring geometry, and readout text.
...
| `[data-ref="control"]` | yes | `aria-valuemin`, `aria-valuemax`, `aria-valuenow`, `aria-valuetext`, `aria-label`, `aria-readonly`, `disabled` | control button; also stamp `data-knob-control` for fields |
| `[data-ref="track"]` | yes | `r`, `stroke-width`, `stroke-dasharray` | SVG track circle |
| `[data-ref="progress"]` | yes | `r`, `stroke-width`, `stroke-dasharray` | SVG progress circle |
| `[data-ref="input"]` | yes | `value`, `name`, `disabled` | hidden form input |
| `[data-ref="label"]` | no | `hidden`, `textContent` | visible label |
| `[data-ref="centerValue"]` | no | `textContent`, `hidden` | in-ring readout |
| `[data-ref="belowValue"]` | no | `textContent`, `hidden` | below-ring readout |

Do not set control `aria-valuenow` or ring geometry. Nested hosts: none.
...
| --- | --- | --- |
| `RuiKnob` | full `[data-ref]` tree under `[data-ref="root"]` | Stamps control, ring SVG, readouts, and hidden input. |
