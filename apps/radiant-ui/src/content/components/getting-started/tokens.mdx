...
| 3. System | Absolute colours (`--color-black`, `--color-white`), border widths, focus ring geometry, opacity, z-index, control and icon sizes | Shared by every theme; not a mood pack. `--size-control-*` aliases the spacing scale. |
...
| Supporting | `--border`, `--focus-ring`, `--on-focus-ring`, `--link`, `--overlay`, `--neutral`, `--background-code`, `--on-background-code` |
| Range tracks | `--rui-track-mix` (`22%`), `--rui-track-fill`; optional `--rui-track-color` |
...
`--size-control-sm`, `--size-control-md`, and `--size-control-lg` alias `--space-8`, `--space-10`, and `--space-12`, so
compact and wide change button and input height on the block axis as well as `--space-control-x` padding.
...
| `--rui-track-mix` | `22%` |
| `--rui-track-fill` | `color-mix` of `--on-background` at `--rui-track-mix` |
...
| `--z-modal` | 40 |
| `--z-toast` | 50 |
| `--size-control-sm` | `var(--space-8)` (2rem / 1.5rem / 2.5rem) |
| `--size-control-md` | `var(--space-10)` (2.5rem / 2rem / 3rem) |
| `--size-control-lg` | `var(--space-12)` (3rem / 2.5rem / 3.75rem) |
| `--size-icon-sm` | 1rem |
