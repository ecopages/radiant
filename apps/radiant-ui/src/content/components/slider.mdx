...
| `[data-ref="rangeTrack"]` | yes | inline `--rui-slider-*`, optional `title` | the track node |
| `[data-thumb]` | yes | `hidden`, `disabled`, `tabindex`, `aria-*` value attrs | thumb elements (`value`, `min`, or `max`) |
| `[data-ref="header"]` | no | `hidden` | header row |
| `[data-ref="label"]` | no | `hidden`, `textContent` | label text |
| `[data-ref="value"]` | no | `textContent` | readout node |
...
Do not set thumb `tabindex` or `aria-valuenow`. Nested hosts: none.
...
| --- | --- | --- |
| `RuiSlider` | full `[data-ref]` tree under `[data-ref="root"]` | Supplies default `RuiSliderValue` with `[data-default-value]` when `children` omitted. |
| `RuiSliderValue` | `[data-ref="value"]` | Live readout; host updates `textContent` while dragging. |
