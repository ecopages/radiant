...
<p>Match `step` to meaningful increments. Fractional steps such as `0.1` are rounded to the step's decimal places, so committed values stay `0.3` instead of `0.30000000000000004`.</p>

<p>Set `precision` when you need a different maximum number of fraction digits than `step` implies. Pass `parseValue` for custom rounding.</p>

```tsx
<RuiSlider min={0} max={1} step={0.1} value={0.3} showValue />

<RuiSlider min={0} max={1} step={0.001} precision={2} showValue />

<RuiSlider min={0} max={1} step={0.001} parseValue={(value) => Number(value.toPrecision(3))} showValue />
```
...
| `precision` | `number` | decimal places in `step` | Maximum fraction digits on committed values. |
...
`parseValue` is a function prop (`(value: number) => number`), not an attribute. Pass it to `RuiSlider`.
