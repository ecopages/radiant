...
`<rui-field>` coordinates any light-DOM tree that matches its query contract. The `RuiField` helper authors a `.rui-field` column; it is not required.
...
| --- | --- | --- |
| `RuiField` | `.rui-field` column | Passes `rules` / `defaultValue` via `prop:` bindings. |
