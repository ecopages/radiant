...
`@query` and `@querySlot` are for DOM you don't own from `render()` — slotted/consumer-authored content, a `RadiantController` enhancing existing markup, or a live `Element` handle needed by a third-party imperative API. They are not a substitute for binding a value your own `render()` JSX already produces.

If a `@query` ref exists only to be written to from an `@onUpdated` callback with a value that could instead be expressed directly in JSX, prefer the binding:
...
<Banner type="alert" class="mb-4 unstyled">
...
<Banner type="alert" class="unstyled">
