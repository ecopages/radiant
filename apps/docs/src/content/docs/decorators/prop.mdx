...
| `transform` | `PropTransform<T>` | Override attribute conversion and optional JS writes. |
...
const csvStrings: PropTransform<string[]> = {
...
- `toAttribute` returning `null` or `''` **omits** the reflected attribute. That is how an empty selection stays off the host tag.
