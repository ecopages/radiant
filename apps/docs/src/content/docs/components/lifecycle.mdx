...
The branches are independent. Use `@bindTo` to copy a field onto existing DOM. Use `@onUpdated()` for a procedure. Use JSX bindings and the render scheduler for DOM your host owns in `render()`. A plain `RadiantElement` that keeps authored children does not enter the render branch unless it overrides `render()`. During first-connect `@prop` synchronization, `@onUpdated()` and `@bindTo` can run before `onConnected()`. `@bindTo` also flushes after catch-up (and the initial hydrate/update) so parent-authored `data-ref` nodes are visible.
...
| `@bindTo(...)` | After catch-up and whenever the named field changes | Copy the field onto host or descendant attributes, properties, or text. |
| `@onUpdated(...)` | When the named reactive property changes, including the initial `@prop` sync | Procedures: focus, timers, joined ARIA, derived state, storage. |
...
5. Radiant flushes `@bindTo` paint, then calls `onConnected()`.
