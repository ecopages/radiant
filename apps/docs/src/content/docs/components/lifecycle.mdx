...
| `updateComplete` | Resolves when no member change or render is pending | Tests and callers that must wait for the cycle, including the first connect render. |
...
| `requestUpdate()` / `update()` | For render-owning hosts | Schedule the update cycle, or run it now. Most components should let reactive JSX bindings schedule this automatically. |
