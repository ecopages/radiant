...
Connection is only the first pass. After a host is connected, a property or observed attribute can trigger `@bindTo` paint, an `@onUpdated()` callback, and, for render-owning hosts, a scheduled `update()`.

The branches are independent. Use `@bindTo` to copy a field onto existing DOM. Use `@onUpdated()` for a procedure. Use JSX bindings and the update cycle for DOM your host owns in `render()`. A plain `RadiantElement` that keeps authored children does not enter the render branch unless it overrides `render()`. During first-connect `@prop` synchronization, `@onUpdated()` and `@bindTo` can run before `onConnected()`. `@bindTo` also flushes after catch-up (and the initial hydrate/update) so parent-authored `data-ref` nodes are visible.
...
| `updated(changed)` | After each update cycle, once `@onUpdated` ran and any pending render committed | Work that needs the committed DOM, such as focus or measuring. State written here schedules another cycle. |
| `updateComplete` | Resolves when no member change or render is pending; rejects when the cycle throws | Tests and callers that must wait for the cycle, including the first connect render. |
| `@bindTo(...)` | After catch-up and on every write of the named field | Copy the field onto host or descendant attributes, properties, or text. |
| `@onUpdated(...)` | Once per update cycle when a watched member changed, before the render commits, including the initial `@prop` sync | Procedures: timers, joined ARIA, derived state, storage. |
| `requestUpdate()` / `update()` | `requestUpdate()` for render-owning hosts; `update()` on any host | Schedule the update cycle, or run it now. Most components should let reactive JSX bindings schedule this automatically. |
...
3. Radiant queues the connection work and catches up authored attributes. A property assigned before that sync wins over the authored attribute.
4. If `render()` is overridden, Radiant hydrates or renders the initial view; otherwise it keeps the authored light DOM.
5. Radiant flushes `@bindTo` paint, calls `onConnected()`, then `updated(changed)`.
...
- Keep authored light DOM in place and copy fields with [@bindTo](/docs/decorators/bind-to). Do not query a node only to copy a reactive value into it.
- Use `@onUpdated('property')` when a procedure must follow property changes (focus, joined ARIA, derived state).
- Render your own UI with JSX bindings when the host owns `render()`.
...
- [@bindTo](/docs/decorators/bind-to) for copying fields onto existing DOM
- [RadiantController](/docs/components/radiant-controller) for attaching the same reactive model to existing HTML
- [Slots](/docs/components/slots) for HTML-first light-DOM projection
