...
Full host HTML (`<my-element>...</my-element>`) is produced by the server pipeline — prefer `renderComponent(...)` from `@ecopages/radiant/server/render-component`. See [Component SSR](/docs/ssr/component-ssr).
...
Most elements do not need this hook. Do not use it to copy a reactive value into JSX-owned DOM; use a JSX binding. Use `@onUpdated(...)` when a property change must synchronize imperative or authored DOM. Reach for `onConnected()` only when first-connect setup genuinely needs the synchronized attributes or committed DOM, and must be repeated after a reconnect. `@query(...)` is unrelated: use it only when that setup needs a live element reference.
