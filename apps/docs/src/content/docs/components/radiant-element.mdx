...
- `@bindTo` for copying fields onto existing light DOM
- update callbacks for procedures that are not a straight DOM copy
...
- `update()` to run the update cycle now: batched `@onUpdated` callbacks, the current JSX view, then `updated()` (without a `render()` override it still flushes `@onUpdated` and `updated()`)
- `requestUpdate()` to schedule that cycle in a microtask
- `updateComplete` to wait until the cycle, including the first connect render, finishes
