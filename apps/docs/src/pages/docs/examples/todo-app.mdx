...
export const getMetadata = () => ({
	...
	description: 'An integrated example combining a RadiantComponent host with provider-backed child components.',
});

# Radiant Todo App

A `RadiantComponent` host that owns a shared todo context, with child components that consume it for local interactions.
...
- `RadiantComponent` child items that consume that context for toggle and remove interactions
...
JSON child text inside `<script type="application/json">` is emitted as raw script text, so the provider can read it back without an `innerHTML` escape hatch.

`stringifyTyped` comes from `@ecopages/radiant/tools/stringify-typed`, not the root entrypoint.
...
Each todo item is a `RadiantComponent` that renders its own view and consumes context for shared state mutations.
