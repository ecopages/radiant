...
<CodeBlock lang="jsx">{`<radiant-todo-app class="todo" hydrate-context={stringifyAttribute<TodoContext>({ todos: data.todos })}>
  <div>
    <article>
      <h4>Todo List</h4>
      <p>
...
<CodeBlock lang='typescript'>{`export type RadiantTodoProps = {
  complete?: boolean;
};
...
<CodeBlock lang='typescript'>{`import type { Todo } from './radiant-todo-app.script';
