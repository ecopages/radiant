...
- Uses the [@reactiveProp](/docs/decorators/reactive-prop) decorator to define a property
- Uses the [@query](/docs/decorators/query) decorator to query for an element
- Uses the [@onEvent](/docs/decorators/on-event) decorator to listen for events
- Uses the [@onUpdated](/docs/decorators/on-updated) decorator to update the text content of the element
