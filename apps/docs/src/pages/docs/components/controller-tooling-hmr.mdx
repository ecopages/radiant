...
export const getMetadata = () => ({
	title: 'Docs | Controller Tooling & HMR',
	description:
		'How RadiantController registration behaves by default, when to use explicit replacement, and where the current Ecopages JSX integration should move upstream.',
});
...
## Why It Is Not In The Plugin Yet

The real upstream home is the Ecopages JSX browser-runtime bootstrap, not each app shell.

Today, `@ecopages/ecopages-jsx` already emits a global Radiant bootstrap script for hydration. That logic lives in the installed package here:

- `node_modules/@ecopages/ecopages-jsx/src/services/jsx-runtime-bundle.service.js`

The current seam is the method named `createRadiantHydratorBootstrapSource()`.

Right now it emits only:
