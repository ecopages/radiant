...
- `renderHostToString({ mode: 'hydrate' })`
- `renderToString({ mode: 'hydrate' })` when you only need the view fragment
...
- the server emitted `RadiantComponent` markup with `mode: 'hydrate'`
