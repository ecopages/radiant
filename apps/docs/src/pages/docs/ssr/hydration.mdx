...
export const getMetadata = () => ({
	title: 'Docs | Hydration',
	description: 'How Radiant and JSX hydration work today, including the explicit client hydrator contract for SSR pages.',
});
...
Radiant now makes that boundary explicit for `RadiantComponent` hosts.
...
const html = renderToString(<App />, { hydrate: true });
...
Use the explicit Radiant hydrator when the server rendered a custom-element host such as `<my-card>...</my-card>` through `RadiantComponent` SSR.
...
If either condition is missing, `RadiantComponent` falls back to a fresh client render.

## The Current Radiant Contract

For a hydratable `RadiantComponent` page today, you need three pieces.
...
```ts
renderToString(view, { hydrate: true });
```

For `RadiantComponent` hosts this usually means one of:

- `renderComponent(...)`
- `renderComponentWithPreview(...)`
- `renderHostToString({ hydrate: true })`
- `renderToString({ hydrate: true })` when you only need the view fragment
...
This sets the global hydration gate that `RadiantComponent` checks on first connect.

## What Happens On First Connect

When the browser upgrades a server-rendered `RadiantComponent` host, the runtime does this:
...
- the server emitted `RadiantComponent` markup with `hydrate: true`
...
- `RadiantComponent` hosts fall back to a fresh client render
...
That means the current safest path for SSR pages is:
...
- `@ecopages/radiant` owns host-level hydration for `RadiantComponent` custom elements.
- Radiant host hydration is now explicit on the client.

## Related Guides

- [RadiantComponent](/docs/components/radiant-component) for the component-host lifecycle
