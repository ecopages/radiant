...
export class ResultsListController extends RadiantController {
	@prop({ type: Array, defaultValue: [] }) declare items: Array<{ id: string; label: string }>;
...
`startControllers(...)` comes from the public subpath `@ecopages/radiant/controller-registry`.
