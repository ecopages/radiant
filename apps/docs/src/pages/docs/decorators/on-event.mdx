...
- `window`: (true) Listen to the event on the window object.
- `document`: (true) Listen to the event on the document object.
...
<div class="banner banner--info mt-4" role="alert">
  ...
  <p>This decorator uses event delegation for `selector` and `ref`, which means it relies on event bubbling.
Therefore, it will not work with events that do not bubble, such as `focus`, `blur`, `load`, `unload`, `scroll`, etc.
For focus and blur events, consider using `focusin` and `focusout` which are similar but do bubble.</p>
</div>
