...
`createQuerySlot` creates a lazy accessor for projected light-DOM elements assigned to a `RadiantComponent` slot. It is the functional equivalent of the [`@querySlot`](/docs/decorators/query-slot) decorator.
...
class InfoCard extends RadiantComponent {
...
| :-------- | :--- | :------- | :---------- |
| `host` | `HTMLElement` | Yes | A `RadiantComponent` or any element that implements `getSlotElement`/`getSlotElements`. |
...
- [Slots](/docs/components/slots) — How slot projection works in RadiantComponent.
