...
When Radiant owns SSR for the host, prefer `renderComponent(...)` or `renderHostToString(...)` so hydration scripts are emitted as raw markup. Avoid placing JSON strings directly inside `<script>` children in JSX, because they will be HTML-escaped before the browser parses them.
