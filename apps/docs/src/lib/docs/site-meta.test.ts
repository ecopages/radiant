...
const origin = 'https://radiant.ecopages.app';

test('absoluteUrl normalizes origin and pathname', () => {
	expect(absoluteUrl('/', origin)).toBe('https://radiant.ecopages.app/');
	expect(absoluteUrl('/docs/getting-started/introduction', origin)).toBe(
		'https://radiant.ecopages.app/docs/getting-started/introduction',
	);
	expect(absoluteUrl('docs/getting-started/introduction', `${origin}/`)).toBe(
		'https://radiant.ecopages.app/docs/getting-started/introduction',
	);
...
test('absoluteImageUrl strips public/ and falls back to the default OG path', () => {
	expect(absoluteImageUrl(undefined, origin)).toBe(`https://radiant.ecopages.app${DEFAULT_OG_IMAGE_PATH}`);
	expect(absoluteImageUrl('public/assets/images/default-og.png', origin)).toBe(
		`https://radiant.ecopages.app${DEFAULT_OG_IMAGE_PATH}`,
	);
...
test('homepageSoftwareApplicationJsonLd names GitHub as sameAs', () => {
	const jsonLd = homepageSoftwareApplicationJsonLd('A light-DOM custom element library.', origin);
	expect(jsonLd['@type']).toBe('SoftwareApplication');
	expect(jsonLd.name).toBe('Radiant');
	expect(jsonLd.url).toBe('https://radiant.ecopages.app/');
