...
async function run(tree: Root): Promise<Root> {
	return (await unified().use(rehypePrettyCopyCompatibility).run(tree)) as Root;
}
...
describe('rehypePrettyCopyCompatibility', () => {
	test('rewrites transformer copy buttons for the MDX JSX renderer', async () => {
		const tree: Root = {
			type: 'root',
			children: [
				{
		...
		const [code] = (await run(tree)).children as Element[];
	...
	test('accepts className arrays and onClick from the hast tree', async () => {
		const tree: Root = {
			type: 'root',
			children: [
				copyButton({
		...
		const [button] = (await run(tree)).children as Element[];
		expect(button.properties).toEqual({
			type: 'button',
			title: 'Copy code',
			'aria-label': 'Copy code',
