...
test('generateLlmDocs writes llms.txt and text exports', async () => {
	...
	try {
		...
		await generateLlmDocs(outputRoot, { baseUrl: 'https://radiant.ecopages.app/', contentRoot });
		...
		expect(llmsTxt).toContain('Defining typed custom elements');
		...
		expect(llmsTxt).toContain('npm install @ecopages/radiant @ecopages/jsx');
		expect(llmsTxt).toContain('/docs/getting-started/installation');
		expect(llmsTxt).toContain('https://radiant.ecopages.app/llms-content/getting-started/introduction.txt');
