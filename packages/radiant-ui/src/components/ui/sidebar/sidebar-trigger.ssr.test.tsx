...
describe('sidebar-trigger helpers', () => {
	...
	it('computes trigger button classes', () => {
		expect(sidebarTriggerButtonClass({ variant: 'ghost', size: 'md', placement: 'header' })).toBe(
			'rui-button rui-button--ghost rui-button--md rui-sidebar__trigger rui-sidebar__trigger--header',
		);
		expect(sidebarTriggerButtonClass({ variant: 'outline', size: 'sm', placement: 'inset' })).toBe(
			'rui-button rui-button--outline rui-button--sm rui-sidebar__trigger rui-sidebar__trigger--inset',
		);
	});
...
describe('RuiSidebarTrigger SSR', () => {
	it('registers the host and paints the toggle button before hydration', () => {
		...
		expect(html).toContain('<rui-sidebar-trigger');
		expect(html).toContain('data-ref="button"');
		expect(html).toContain('rui-sidebar__trigger--inset');
