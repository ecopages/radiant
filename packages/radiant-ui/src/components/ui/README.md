...
`rui-sidebar-trigger` mirrors its controlled sidebar's state, mobile mode, and collapse mode onto the trigger host through one `MutationObserver`, attached in `onConnected()`. Placement and glyph CSS read only those host attributes and the reflected `placement` attribute, so an attached trigger follows the sidebar's `mobileBreakpoint`, not the viewport. Until a trigger attaches (no `data-sidebar-mobile`), CSS guesses mobile from a 768px viewport and reads the SSR attributes of the sidebar whose pane holds the trigger, else of the only sidebar of its nearest provider (child combinators keep nested providers independent); other triggers stay visible until they attach. Detached, the trigger reports the SSR placement guess (`inset` collapsed, otherwise expanded).
