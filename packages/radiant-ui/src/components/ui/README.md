...
Native `FormData` comes from the listed control, not from `RuiField`. Named `rui-date-input`, `rui-number-field`, `rui-slider`, and `rui-knob` are form-associated (`name` on the host, like `<input name>`). `RuiField` copies its `name` onto that host. `registerFieldControl` is the `RuiForm` store adapter only; a third-party host still needs `FormAssociation` and `static get formAssociated()` to join a native form.
