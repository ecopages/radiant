...
export const RuiAlert = defineRadiantView(
	...
	({
		...
		children,
		...props
	}: JsxHtmlPropsWithChildren<RuiAlertProps & { slot?: string }>) => (
		<rui-alert {...props} variant={variant} layout={layout}>
			<div class={cx('rui-alert', `rui-alert--${variant}`, `rui-alert--${layout}`, className)} role="alert">
				{children}
			</div>
		</rui-alert>
	),
