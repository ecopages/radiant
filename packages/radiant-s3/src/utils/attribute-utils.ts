...
export type ReaderTypeNames = 'array' | 'boolean' | 'number' | 'object' | 'string';

export type WriterTypeNames = 'array' | 'object' | 'string';
 ...
 */
export function parseAttributeTypeConstant(constant?: AttributeTypeConstant): ReaderTypeNames {
  switch (constant) {
    case Array:
      return 'array';
    case Boolean:
 ...
 */
const readers: { [K in ReaderTypeNames]: Reader } = {
 ...
 */
const writers: { [K in WriterTypeNames]: Writer } = {
  string: writeString,
  array: writeJSON,
  object: writeJSON,
};
...
export function writeAttributeValue(value: unknown, type: AttributeTypeConstant) {
  const writerType = parseAttributeTypeConstant(type) as WriterTypeNames;
  if (!writerType) throw new TypeError(`[radiant-element] Unknown type "${type}"`);
  return (writers[writerType] || writers.string)(value);
}
