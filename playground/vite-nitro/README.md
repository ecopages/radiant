...
The playground resolves `@ecopages/*` imports through the workspace-linked package dependencies declared in `package.json`.
...
Use the repo-level Nitro playground dev flow so the library watch/build process runs alongside the playground server.
...
- fragment endpoints now ship a plugin-managed client module reference with the SSR markup so lazy components register through Vite before insertion
